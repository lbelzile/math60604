...
head(renfe, n = 5)  %>% kbl(caption = "Aperçu des données $\\texttt{renfe}$.") %>% kable_styling()
...
renfe %>% count(classe) %>% kbl(caption = "Nombre d'observations du jeu de données $\\texttt{renfe}$ par classe.") %>% kable_styling()
```

```{r renfe-aed1b, echo = FALSE}
# un raccourci pour la même syntaxe
renfe %>% group_by(type) %>% tally()  %>% kbl(caption = "Nombre d'observations du jeu de données $\\texttt{renfe}$ par type de billets.") %>% kable_styling()
```

```{r renfe-aed1c, echo = FALSE}
renfe %>% group_by(tarif) %>% tally()  %>% kbl(caption = "Nombre d'observations du jeu de données $\\texttt{renfe}$ par tarif.") %>% kable_styling()
```
...
renfe %>% count(tarif, type)  %>% kbl(caption = "Tableau croisé du décompte par tarif et type de billets du jeu de données $\\texttt{renfe}$.") %>% kable_styling()
```
...
renfe %>%
  ...
  summarise("durée moyenne" = mean(duree),
            ...
            "écart-type" = sd(prix)) %>%
            kbl(caption = "Statistiques descriptives de la durée de trajet et du prix du billet par type de billet et destination pour le jeu de données $\\texttt{renfe}$.") %>% kable_styling()
```
...
renfe %>%
  ...
  summarise("durée moyenne" = mean(duree),
            ...
            "prix moyen" = mean(prix),
            "écart-type" = sd(prix)) %>%
  kbl(caption = "Statistiques descriptives de la durée de trajet et du prix du billet par type de billet et destination pour les trains grande vitesse du jeu de données $\\texttt{renfe}$.") %>% kable_styling()
```
...
renfe %>% subset(tarif  == "Flexible") %>% count(prix, classe)  %>% kbl(caption = "Décompte des billets au tarif Flexible par prix et classe pour le jeu de données $\\texttt{renfe}$.") %>% kable_styling()
```
