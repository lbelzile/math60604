...
\end{align*}
Puisque $\widehat{\sigma}^2=(n-1)/n S^2,$ il s'ensuit que l'estimateur du maximum de vraisemblance de $\sigma^2$ est biaisé, mais les deux estimateurs sont cohérents et s'approcheront donc de la vraie valeur $\sigma^2$ pour $n$ suffisamment grand.
...
\end{align*}
Si on dénote par $\widehat{\boldsymbol{y}}=\mathbf{X}\widehat{\boldsymbol{\beta}}$ les prédictions du modèle, maximiser la log-vraisemblance par rapport à $\boldsymbol{\beta}$ équivaut à
...
\end{align*}
En dérivant chaque terme par rapport à $\sigma^2$ et en fixant le gradient à zéro, on obtient l'estimateur du maximum de vraisemblance
\begin{align*}
...
D'où provient cette loi $\chi^2_q$? Considérons pour faire simple le cas où $q=2$ restrictions sont imposées dans un modèle avec $p=2$ paramètres (par ex., la loi de Weibull) et le test de Wald. La loi asymptotique de l'estimateur du maximum de vraisemblance dicte que $\widehat{\boldsymbol{\theta}} \stackrel{\cdot}{\sim} \mathsf{normale}_p\{\boldsymbol{\theta}, i^{-1}(\boldsymbol{\theta})\},$ et on peut obtenir des ellipses de confiance pour un test en remplaçant l'information de Fisher $i(\boldsymbol{\theta})$ par un estimateur cohérent comme $j(\widehat{\boldsymbol{\theta}})$: on obtient alors le panneau de gauche de la @fig-ellipses. 
...
## Comparaison de modèles non-emboîtés pour les temps d'attente

On a considéré deux familles de lois paramétriques, soit exponentielle et Weibull, pour les temps d'attente. D'autres lois pour des données continues et positives incluent la loi lognormale et la loi gamma.
