...
\end{align*}
L'espace d'échantillonage est $\mathbb{R}_{+}^n = [0, \infty)^n,$ et l'espace des paramètres est $(0, \infty).$
:::
...
On peut utiliser ce résultat pour obtenir une approximation à la loi d'échantillonage des estimateurs du maximum de vraisemblance de $\widehat{\boldsymbol{\theta}},$ 
\begin{align*}
...
## Tests dérivés de la vraisemblance {#testsvrais}
...
g1 <- ggplot() +
  stat_qq(data = data.frame(y = attente), 
          mapping = aes(sample = y),
          loi = qexp, dparams  = list(rate = 1 / mean(attente))) +
...
Parfois, le postulat de normalité de l'erreur dans une régression linéaire ne tient pas. Si les données sont strictement positives, on peut envisager une transformation de Box-Cox,
\begin{align*}
...
Si on postule que $\boldsymbol{Y}(\lambda) \sim \mathsf{normale}(\mathbf{X}\boldsymbol{\beta}, \sigma^2 \mathbf{I}_n)$, alors la log-vraisemblance s'écrit
\begin{align*}
