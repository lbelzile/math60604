...
\NormalTok{url \textless{}{-}}\StringTok{ "https://lbelzile.bitbucket.io/MATH60604/renfe.sas7bdat"}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\#Identifier les variables catégorielles}
\NormalTok{renfe \textless{}{-}}\StringTok{ }\NormalTok{renfe }\OperatorTok{\%\textgreater{}\%}\StringTok{ }
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{dest =} \KeywordTok{factor}\NormalTok{(}\KeywordTok{recode}\NormalTok{(dest, }\StringTok{"0"}\NormalTok{ =}\StringTok{ "Barcelone{-}Madrid"}\NormalTok{,}
               \StringTok{"1"}\NormalTok{ =}\StringTok{ "Madrid{-}Barcelone"}\NormalTok{)),}
         \DataTypeTok{type =} \KeywordTok{factor}\NormalTok{(type),}
         \DataTypeTok{dest =} \KeywordTok{factor}\NormalTok{(dest),}
         \DataTypeTok{jour =} \KeywordTok{factor}\NormalTok{(jour),}
         \DataTypeTok{tarif =} \KeywordTok{factor}\NormalTok{(tarif),}
         \DataTypeTok{classe =} \KeywordTok{factor}\NormalTok{(classe))}
...
Dans un rapport, un résumé des caractéristiques les plus importantes devrait être inclut pour que le lecteur ou la lectrice puisse valider l'interprétation.
...
{\centering \includegraphics[width=0.7\linewidth]{images/continuous_discrete} 
...
{\centering \includegraphics[width=0.7\linewidth]{images/nominal_ordinal_binary} 
...
\caption{Diagramme à bande pour la classe des billets de trains du jeu de données Renfe}\label{fig:barplotrenfe}
\end{figure}
...
Une boîte-à-moustache (\emph{boxplot}) représente graphiquement cinq statistiques descriptives.
...
\caption{Boîte à moustache du prix des billets au tarif Promo en fonction de la classe pour le jeu de données Renfe}\label{fig:histboxplot}
\end{figure}
...
Par exemple, la Figure \ref{fig:nuagedepoints} montre le temps de réaction d'individus participant à une expérience effectuée au Tech3Lab; on voit que le temps de réaction pour une personne qui texte est plus lent que lors d'une conversation téléphonique. La corrélation linéaire entre ces données appariées est faible.
...
\caption{Nuage de points du temps de réaction à deux tâches (données appariées) pour les données distractions}\label{fig:nuagedepoints}
\end{figure}
...
\caption{Boîte à moustache du prix de billets de train de Renfe en fonction de la destination et du type de train.}\label{fig:renfe-aed4}
\end{figure}
...
\caption{Boîte à moustache du prix en fonction du tarif et de la classe de billets de trains à haute vitesse de la Renfe.}\label{fig:renfe-aed6}
\end{figure}
...
\subsection{Population et échantillons}\label{population-et-uxe9chantillons}}
