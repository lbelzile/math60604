...
\item
  \texttt{annees}: nombre d'années de service au sein de l'échelon académique (rang).
\end{itemize}
...
Le modèle de régression linéaire simple, qui n'inclut qu'une variable explicative, consiste en une droite qui passe à travers un nuage de points et capture la corrélation linéaire entre deux variables \(\mathrm{X}\) et \(Y\) à l'aide d'une équation de la forme \(y=a+bx\)? En deux dimensions, le problème de la régression linéaire revient à ajuster la meilleure droite (selon un critère donné) à travers un nuage de points comme celui de la Figure \ref{fig:droitenuage}.
...
\end{align*}
en utilisant la règle de dérivation en chaîne; on peut ainsi distribuer les termes pour obtenir l'\emph{équation normale}
\begin{align*}
 \mathbf{X}^\top \mathbf{X}\boldsymbol{\beta}&=\mathbf{X}^\top \boldsymbol{y}.
\end{align*}
Si \(\mathbf{X}\) est une matrice de rang \(p\), alors la forme quadratique \(\mathbf{X}^\top \mathbf{X}\) est inversible et on obtient ainsi une formule explicite pour l'estimateur des moindres carrés,
\begin{align*}
\widehat{\boldsymbol{\beta}} = (\mathbf{X}^{\top} \mathbf{X})^{-1} \mathbf{X}^{\top} \boldsymbol{Y}.
\end{align*}

Que représente les moindres carrés en deux dimensions? L'estimateur est celui qui minimise la somme du carré des résidus ordinaires. Le \(i\)e \textbf{résidu} ordinaire \(e_i = y_i -\widehat{y}_i\) est la distance \textbf{verticale} entre un point \(y_i\) et la valeur ajustée \(\widehat{y}_i\), soit les traits bleus de la Figure \ref{fig:distancevert}.
...
\caption{Représentation graphique 3D du modèle de régression linéaire pour les données automobile}\label{fig:hyperplan}
\end{figure}
