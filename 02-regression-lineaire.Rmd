...
-   `annees`: nombre d'années de service au sein de l'échelon académique (rang).
...
```{r edacollege, eval = TRUE, echo = FALSE, fig.cap = "Analyse exploratoire des données $\\texttt{college}$: répartition des salaires en fonction de l'échelon et du nombre d'années de service"}
...
<!-- Ajouter l'enfilade de gazouillis de Daniela Witten - https://threadreaderapp.com/thread/1286420597505892352.html-->
...
\end{align*}
Ce système d'équation a une solution unique et explicite qui est plus facilement exprimée en notation matricielle: cela revient à exprimer le système linéaire de l'équation \@ref(eq:olsmean) avec une observation par ligne. 
...
Dans le contexte de notre exemple, l'équation de la droite ajustée de la Figure \@ref(fig:droitenuage) est \[\widehat{\texttt{salaire}} = `r coef(lmprof)[1]` + `r coef(lmprof)[2]`\texttt{service}.\]
...
Si la variable réponse $Y$ doit être *continue*, il n'y a aucune restriction pour les variables explicatives. Le cas des variables explicatives binaires est illustratif: ces variables sont encodées numériquement à l'aide de 0/1. Considérons par exemple le sexe des professeurs de l'étude, par exemple
...
```{r graphcollegesexe, eval = TRUE, echo = FALSE, fig.cap = "Modèle linéaire simple pour les données $\\texttt{college}$ en fonction de la variable binaire sexe: bien que le modèle définisse une ligne, seule la valeur en $0/1$ est réalisable."}
...
```{r hyperplan, echo = FALSE, fig.cap = "Représentation graphique 3D du modèle de régression linéaire pour les données automobile"}
