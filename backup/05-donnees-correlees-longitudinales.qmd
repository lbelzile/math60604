...
```{example}
#| label: sixvilles
#| name: Étude longitudinale sur l'effet de la pollution atmosphérique sur la mortalité
#|   dans six villes américaines.
...
```{r}
#| label: diagrammespagh1
#| echo: false
#| cache: true
#| eval: true
#| fig-cap: Diagramme spaghetti des données de spirométrie de l'étude *Six Cities Study
...
```{r}
#| label: diagrammespagh2
#| echo: false
#| cache: true
#| eval: true
#| fig-cap: Diagramme spaghetti  pour les données de l'étude clinique sur la dépression
#|   et du programme comportemental *Beating the Blues*.
...
```{example}
#| label: beatingtheblues
#| name: Programme thérapeutique *Beating the Blues*.
...
#| label: longvswide
#| echo: false
#| fig-cap: Format large versus format long pour tableaux (illustration de Garrick Aden-Buie).
...
#| eval: true
#| cache: true
#| fig-cap: Matrice de covariance des 100 premières mesures de l'étude sur la capacité
#|   respiratoire pour un modèle donné. Chaque bloc représente une fille différente.
data(fev1, package = "hecmodstat")
...
#| eval: true
#| cache: true
#| fig-cap: Estimés de la matrice de corrélation pour l'individu 6 avec les données *Beating
#|   the Blues*, avec modèle de covariance non structurée (gauche) et modèle autorégressif
#|   hétérogène d'ordre 1 (droite).
mod1 <- nlme::gls(
