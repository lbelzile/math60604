...
Ce chapitre couvre des rappels mathématiques et porte sur deux concepts fondamentaux pour la modélisation, à savoir les principes sous-jacents aux tests d'hypothèses.
...
On dénote les variables aléatoires par des lettres majuscules, et leurs réalisations par des minuscules: par exemple, $Y \sim \mathsf{No}(\mu, \sigma^2)$ indique que $Y$ suit une loi normale de paramètres $\mu$ et $\sigma$, qui représentent respectivement l'espérance et l'écart-type de $Y$. 
...
Si la variable $Y$ est continue, aucune valeur numérique de $y$ n'a de probabilité non-nulle et $\Pr(Y=y) = 0$ pour toute valeur réelle $y$; la densité sert à estimer la probabilité que la variable $Y$ appartienne à un ensemble $B$, via $\mathsf{Pr}(Y \in B) = \int_B f(y) \mathrm{d} y$; la fonction de répartition est ainsi $F(y) = \int_{-\infty}^y f(x) \mathrm{d} x$. 
...
Soit $Y$ une variable aléatoire de fonction de densité (ou de masse) $f(x)$. Cette fonction est non-négative et satisfait $\int_{\mathbb{R}} f(x) \mathrm{d}x=1$: elle décrit la probabilité d'obtenir un résultat dans un ensemble donné des réels $\mathbb{R}$, pour n'importe quel intervalle.
...
# Charger les données
data(renfe, package = "hecmodstat")
head(renfe, n = 5)  %>% kbl(booktabs = TRUE, caption = "Aperçu des données $\\texttt{renfe}$.") %>% kable_styling()
...
```{r renfepermut, cache = TRUE, echo = FALSE, fig.cap="Approximation par permutation de la loi nulle de la statistique de test de Welch (histogramme et trait noir) et loi asymptotique normale standard (trait bleu) pour le prix de billets de trains AVE au tarif promotionnel entre Madrid et Barcelone. La valeur de la statistique de test de l'échantillon original est représentée par un trait vertical.", fig.width = 6, fig.height = 5}
...
```{r nuagedepoints, cache = TRUE, eval = FALSE, echo = FALSE, fig.cap="Nuage de points (gauche) et diagramme hexagonal (droite) pour des données simulées.", fig.width = 8}
set.seed(1234)
...
-->
```{r seriechrono, cache = TRUE, eval = FALSE, fig.cap = "Représentation graphique d'une série chronologique.", echo = FALSE}
set.seed(1234)
# Dummy data
data <- data.frame(
...
renfe %>% count(tarif, type)  %>% kbl(booktabs = TRUE, caption = "Tableau croisé du décompte par tarif et type de billets du jeu de données $\\texttt{renfe}$.") %>% kable_styling()
```
...
renfe %>%
  ...
  summarise("durée moyenne" = mean(duree),
            ...
            "écart-type" = sd(prix)) %>%
            kbl(booktabs = TRUE, caption = "Statistiques descriptives de la durée de trajet et du prix du billet par type de billet et destination pour le jeu de données $\\texttt{renfe}$.") %>% kable_styling()
```
...
renfe %>%
  ...
  summarise("durée moyenne" = mean(duree),
            ...
            "prix moyen" = mean(prix),
            "écart-type" = sd(prix)) %>%
  kbl(booktabs = TRUE, caption = "Statistiques descriptives de la durée de trajet et du prix du billet par type de billet et destination pour les trains à grande vitesse du jeu de données $\\texttt{renfe}$.") %>% kable_styling()
```
...
ggplot(data = renfe, aes(x = prix, y=after_stat(density), fill = tarif)) +
    geom_histogram(binwidth = 5) +
    labs(x = "prix (en euros)", y = "densité") +
    theme(legend.position = "bottom")
# Vérifier la répartition des billets Flexible
renfe %>% subset(tarif  == "Flexible") %>% count(prix, classe)  %>% kbl(booktabs = TRUE, caption = "Décompte des billets au tarif Flexible par prix et classe pour le jeu de données $\\texttt{renfe}$.") %>% kable_styling()
```
